...
class TweetQuery:
    ...
    def query_tweets_by_regex(self, regex_pattern):
        ...
        tweets = self.coll_tweets.find({"text": {"$regex": regex_pattern, "$options": "i"}}, {'_id':0}).sort("popularity", pymongo.DESCENDING).limit(10)
    ...
    def get_comments_for_tweet(self, tweet_id):
        comments = self.coll_comments.find({"in_reply_to_status_id_str": tweet_id},{'id':0}).sort("created_at", pymongo.ASCENDING)
        return self.get_details_list(comments)

    def get_tweets_by_username(self, username):
        tweets = self.coll_tweets.find({"user_name": username},{'_id':0})
        return self.get_details_list(tweets)
